...
def home(request):
    ...
    gender_year_counts = Student.objects.filter(verified=True).values('gender', 'dateField__year','dateField__month').annotate(count=Count('id'))
    ...
    context = {
        ...
        'month_list': list(set([item['month'] for item in data])),
